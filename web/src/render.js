  ...
  function createRenderer(canvas) {
    ...
    function buildCache() {
      ...
      for (let ty = 0; ty < rows; ty++) for (let tx = 0; tx < cols; tx++) {
        ...
        if (!s) for (let y = T - H; y < T; y++) for (let x = 0; x < T; x++) { const cutF = (!w && x + (T - 1 - y) < 6) || (!e && (T - 1 - x) + (T - 1 - y) < 6); if (!cutF) solid[(b.oy + y) * W + b.ox + x] = 1; }
      }
      // dirt trail along the route: a thin core that always connects, plus a wider band that keeps clear of blocks
      const pts = [];
      ...
      const along = (ax, ay, bx, by, fn) => { for (let s = 0; s <= 10; s++) { const t = s / 10; fn(Math.round(PAD + (ax + 0.5) * T + (bx - ax) * T * t), Math.round(PAD + (ay + 0.5) * T + (by - ay) * T * t)); } };
      for (let i = 1; i < pts.length; i++) along(pts[i - 1][0], pts[i - 1][1], pts[i][0], pts[i][1], (x, y) => { stampDisc(trail, x, y, 6 + (hash2(x, y) < 0.4 ? 1 : 0)); stampDisc(core, x, y, 2); });
      if (pts.length === 1) stampDisc(trail, PAD + Math.round((pts[0][0] + 0.5) * T), PAD + Math.round((pts[0][1] + 0.5) * T), 7);
      ...
      for (const b of blocks) {
        ...
        if (!b.s) for (let y = T - H; y < T; y++) for (let x = 0; x < T; x++) {
          if ((!b.w && x + (T - 1 - y) < 6) || (!b.e && (T - 1 - x) + (T - 1 - y) < 6)) continue;
          let c;
