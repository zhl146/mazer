...
(function (root) {
  ...
  function createRenderer(canvas) {
    ...
    function shapeOf(tx, ty) {
      ...
      const cut = (x, y) => (!n && !w && x + y < CH) || (!n && !e && (T - 1 - x) + y < CH) || (!s && !w && x + (T - 1 - y) < CH) || (!s && !e && (T - 1 - x) + (T - 1 - y) < CH);
      const bottomRow = (x) => Math.min(T - 1, !s && !w && x < CH ? T - 1 - CH + x : T - 1, !s && !e && x > T - 1 - CH ? T - 1 - CH + (T - 1 - x) : T - 1);
      return { n, e, s, w, cut, bottomRow };
    }
    ...
    function solidAt(x, y) {
      ...
      for (let ty = ty0; ty <= ty0 + 1; ty++) {
        ...
        if (ly >= 0 && ly < T && !sh.cut(lx, ly)) return true;
        if (!sh.s) { const br = sh.bottomRow(lx); if (ly > br && ly <= br + H) return true; }
      }
      return false;
    }
    ...
    function buildCache() {
      ...
      for (let ty = 0; ty < rows; ty++) for (let tx = 0; tx < cols; tx++) {
        if (!isWall(tx, ty)) continue;
        const { n, e, s, w, cut, bottomRow } = shapeOf(tx, ty);
        const b = { tx, ty, n, e, s, w, nat: game.natural[ty * cols + tx] === 1, cut, bottomRow, ox: PAD + tx * T, oy: PAD + ty * T };
        blocks.push(b);
      ...
      for (const b of blocks) {
        ...
        for (let y = 0; y < T; y++) for (let x = 0; x < T; x++) {
          ...
          if (b.nat) { // natural highland: earthy grass, rock outcrops and dirt patches from multi-scale noise
            ...
            if (!b.n && y === 0) c = mix(c, [255, 250, 232], 0.25); if (!b.w && x === 0) c = mix(c, [255, 250, 232], 0.18);
          } else { // log ends
