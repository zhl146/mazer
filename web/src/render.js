...
(function (root) {
  ...
  const CH = 8;       // chamfer on convex corners: shows the diagonal gap the courier slips through
  const INK = [30, 23, 16];
  ...
  function createRenderer(canvas) {
    ...
    function buildCache() {
      ...
      const set = (x, y, c, k) => { if (x < 0 || y < 0 || x >= W || y >= Hh) return; const i = y * W + x; d[i * 4] = c[0]; d[i * 4 + 1] = c[1]; d[i * 4 + 2] = c[2]; d[i * 4 + 3] = 255; if (k !== undefined) cat[i] = k; };
      ...
      const blit = (sp, ox, oy, k) => { for (let y = 0; y < sp.h; y++) for (let x = 0; x < sp.w; x++) { const c = sp.c[y * sp.w + x]; if (c) set(ox + x, oy + y, c, k); } };
      ...
      const stampDisc = (cx, cy, r) => { for (let y = -r; y <= r; y++) for (let x = -r; x <= r; x++) if (x * x + y * y <= r * r) { const px = cx + x, py = cy + y; if (px >= 0 && py >= 0 && px < W && py < Hh) trail[py * W + px] = 1; } };
      for (let i = 1; i < pts.length; i++) {
        const [ax, ay] = pts[i - 1], [bx, by] = pts[i];
        for (let s = 0; s <= 8; s++) { const t = s / 8, x = PAD + (ax + 0.5) * T + (bx - ax) * T * t, y = PAD + (ay + 0.5) * T + (by - ay) * T * t; stampDisc(Math.round(x), Math.round(y), 6 + (hash2(Math.round(x), Math.round(y)) < 0.4 ? 1 : 0)); }
      }
      if (pts.length === 1) stampDisc(PAD + Math.round((pts[0][0] + 0.5) * T), PAD + Math.round((pts[0][1] + 0.5) * T), 7);
      for (let y = 0; y < Hh; y++) for (let x = 0; x < W; x++) if (trail[y * W + x]) { const h = hash2(x * 3, y * 7); set(x, y, h < 0.15 ? TRAIL[1] : h < 0.22 ? TRAIL[2] : TRAIL[0], 5); }
      ...
      for (let ty = 0; ty < rows; ty++) for (let tx = 0; tx < cols; tx++) {
        ...
        const b = { tx, ty, n, e, s, w, nat: game.natural[ty * cols + tx] === 1, cut: corner(n, e, s, w), ox: PAD + tx * T, oy: PAD + ty * T };
        blocks.push(b);
        for (let y = 0; y < T; y++) for (let x = 0; x < T; x++) if (!b.cut(x, y)) solid[(b.oy + y - H) * W + b.ox + x] = 1;
        if (!s) for (let y = T - H; y < T; y++) for (let x = 0; x < T; x++) { const cutF = (!w && x + (T - 1 - y) < 4) || (!e && (T - 1 - x) + (T - 1 - y) < 4); if (!cutF) solid[(b.oy + y) * W + b.ox + x] = 1; }
      }
      ...
      for (const b of blocks) {
        ...
        if (!b.s) for (let y = T - H; y < T; y++) for (let x = 0; x < T; x++) {
          if ((!b.w && x + (T - 1 - y) < 4) || (!b.e && (T - 1 - x) + (T - 1 - y) < 4)) continue;
          let c;
          if (b.nat) { const st = hash2(b.tx * 32 + x, 3), c0 = st < 0.3 ? R[0] : st < 0.6 ? R[1] : R[2]; c = y >= T - 2 ? R[0] : c0; if (hash2(x + b.tx, y + b.ty) < 0.06) c = R[3]; }
          else { const k = x % 6; c = k === 0 ? R[0] : k === 2 ? R[3] : R[2]; if (y >= T - 2) c = R[0]; }
          set(b.ox + x, b.oy + y, c, b.nat ? 2 : 4);
        }
        for (let y = 0; y < T; y++) for (let x = 0; x < T; x++) {
          ...
          set(b.ox + x, b.oy + y - H, c, b.nat ? 1 : 3);
        }
      }
      ...
      for (const b of blocks) {
        ...
        blit(sp, b.ox + 1, b.oy - H - 24, 6);
      }
      game.waypoints.forEach(([x, y], i) => {
        const ox = PAD + x * T, oy = PAD + y * T, last = i === game.waypoints.length - 1;
        if (i === 0) { propShadow(ox + 18, oy + 22, 17, 5); blit(props.cottage, ox - 2, oy - 14, 6); }
        else if (last) { propShadow(ox + 16, oy + 22, 11, 4); blit(props.tower, ox + 3, oy - 34, 6); }
        else { propShadow(ox + 16, oy + 22, 12, 4); blit(props.well, ox + 2, oy - 4, 6); }
      });
      // zone badges
    ...
    /* ---------- route pixels (for the courier and the error flash) ---------- */
    function buildPathPixels() {
      pathPx = [];
      const pts = [];
      for (let i = 0; i < segments.length; i++) for (let k = i === 0 ? 0 : 1; k < segments[i].length; k++) pts.push(segments[i][k]);
      for (let i = 1; i < pts.length; i++) {
        let x0 = pts[i - 1][0] * T + 16, y0 = pts[i - 1][1] * T + 16;
        const x1 = pts[i][0] * T + 16, y1 = pts[i][1] * T + 16;
        const dx = Math.abs(x1 - x0), dy = -Math.abs(y1 - y0), sx = x0 < x1 ? 1 : -1, sy = y0 < y1 ? 1 : -1; let err = dx + dy;
        for (;;) {
      ...
      if (!pathPx.length && pts.length) pathPx.push([pts[0][0] * T + 16, pts[0][1] * T + 16, 1]);
