...
(function (root) {
  ...
  const CF = 8;       // chamfer on the bottom corners of a cliff face
  const INK = [30, 23, 16];
  ...
  function createRenderer(canvas) {
    ...
    function buildCache() {
      ...
      for (let ty = 0; ty < rows; ty++) for (let tx = 0; tx < cols; tx++) {
        ...
        if (!s) for (let y = T - H; y < T; y++) for (let x = 0; x < T; x++) { const cutF = (!w && x + (T - 1 - y) < CF) || (!e && (T - 1 - x) + (T - 1 - y) < CF); if (!cutF) solid[(b.oy + y) * W + b.ox + x] = 1; }
      }
      ...
      for (const b of blocks) {
        ...
        if (!b.s) for (let y = T - H; y < T; y++) for (let x = 0; x < T; x++) {
          if ((!b.w && x + (T - 1 - y) < CF) || (!b.e && (T - 1 - x) + (T - 1 - y) < CF)) continue;
          let c;
          ...
          set(b.ox + x, b.oy + y, c, b.nat ? 2 : 4, b.ty);
        }
