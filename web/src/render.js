...
(function (root) {
  ...
  const RISE = 40;    // how far beacons, canopies and waypoint labels reach above the board's top row
  const INK = [30, 23, 16];
  ...
  function paintMenhir(n) {
    ...
    // the stop's number is cut into the face: a recess with a lit floor, so it reads without a label box
    const txt = String(n), sc = 2, cut = [];
    pixelText((x, y, w, h) => { for (let j = 0; j < h; j++) for (let i = 0; i < w; i++) cut.push([x + i, y + j]); }, txt, 14 - Math.floor(textWidth(txt, sc) / 2), 22, sc, 1, null);
    const carved = new Set(cut.map(([x, y]) => y * 28 + x));
    let x0 = 99, y0 = 99, x1 = -1, y1 = -1;
    for (const [x, y] of cut) { if (x < x0) x0 = x; if (y < y0) y0 = y; if (x > x1) x1 = x; if (y > y1) y1 = y; }
    for (let y = y0 - 1; y <= y1 + 1; y++) for (let x = x0 - 1; x <= x1 + 1; x++) { // a dressed panel: flat stone for the mason to cut into
      if (sp.get(x, y) && !carved.has(y * 28 + x)) sp.set(x, y, hash2(x, y) < 0.18 ? GRANITE.M : GRANITE.L);
    }
    for (const [x, y] of cut) if (sp.get(x, y)) sp.set(x, y, [32, 40, 48]); // the cut itself: two tones only, so the digit never reads as stripes
  ...
  function createRenderer(canvas) {
    ...
    function buildProps() {
      ...
      props = {
        ...
        focus: paintFocus(), beacon: paintBeacon(), menhirs: game.waypoints.slice(1, -1).map((_, i) => paintMenhir(i + 1)),
    ...
    function buildCache() {
      ...
      for (const b of blocks) {
        ...
        if (!b.s) for (let x = 0; x < T; x++) { // cliff face: extruded down from the plateau's (wobbly) bottom edge
          ...
          for (let k = 1; k <= fh; k++) {
            ...
            if (b.oy - H + y >= PAD) set(b.ox + x, b.oy - H + y, c, b.nat ? 2 : 4, b.ty); // the frame bounds the ground plane
          }
        }
        for (let y = 0; y < T; y++) for (let x = 0; x < T; x++) {
          ...
          if (b.oy + y - H >= PAD) set(b.ox + x, b.oy + y - H, c, b.nat ? 1 : 3, b.ty);
        }
      }
      ...
      for (const b of blocks) {
        ...
        if (h1 < 0.22) { // boulder
          ...
          for (let dy = -Math.ceil(ry) - 1; dy <= Math.ceil(ry) + 1; dy++) for (let dx = -Math.ceil(rx) - 1; dx <= Math.ceil(rx) + 1; dx++) {
            ...
            if (py + dy >= PAD) set(px + dx, py + dy, c, 1, b.ty);
          }
          ...
          for (let k = 0; k < 3; k++) { const hh = 2 + Math.floor(hash2(px + k, py) * 3); for (let j = 0; j <= hh; j++) if (py - j >= PAD) set(px + k * 2 - 2, py - j, j === hh ? P.ground[3] : P.ground[1], 1, b.ty); }
        }
      }
      ...
      game.waypoints.forEach(([x, y], i) => {
        ...
        else { propShadow(ox + 16, oy + 24, 10, 4); blit(props.menhirs[i - 1], ox + 2, oy - 14, 6, y); }
      });
      // zone badges
      const plotBuf = (x, y, w, h, c) => { for (let yy = 0; yy < h; yy++) for (let xx = 0; xx < w; xx++) set(x + xx, y + yy, c); };
      const badged = new Set();
      for (const z of game.zones) { // a ring's centre tile is a hole, so badge the strongest charged tile instead
        let best = null, bestM = 1, bestD = Infinity;
        for (let ty = Math.max(0, z.y - z.r); ty <= Math.min(rows - 1, z.y + z.r); ty++)
          for (let tx = Math.max(0, z.x - z.r); tx <= Math.min(cols - 1, z.x + z.r); tx++) {
            const m = game.scoreMod[ty * cols + tx]; if (m <= 1) continue;
            const dx = tx - z.x, dy = ty - z.y, d = dx * dx + dy * dy;
            if (m > bestM || (m === bestM && d < bestD)) { bestM = m; bestD = d; best = [tx, ty]; }
          }
        if (!best) continue;
        const key = best[1] * cols + best[0]; if (badged.has(key)) continue;
        badged.add(key);
        const m = Math.round(bestM), txt = "x" + m, tw = textWidth(txt, 2);
        const bx = PAD + best[0] * T + 16 - Math.floor(tw / 2), by = PAD + best[1] * T + 11;
        plotBuf(bx - 3, by - 3, tw + 6, 16, INK); pixelText(plotBuf, txt, bx, by, 2, [255, 240, 200], null);
      }
      // charged ground: a breathing tint plus a lit border, so the bonus area reads at a glance
      zoneFx = null;
      {
        let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
        const charged = (tx, ty) => tx >= 0 && ty >= 0 && tx < cols && ty < rows && game.scoreMod[ty * cols + tx] > 1;
        for (let ty = 0; ty < rows; ty++) for (let tx = 0; tx < cols; tx++) {
          if (!charged(tx, ty)) continue;
          if (tx < x0) x0 = tx; if (ty < y0) y0 = ty; if (tx > x1) x1 = tx; if (ty > y1) y1 = ty;
        }
        if (x0 <= x1) {
          const fw = (x1 - x0 + 1) * T, fh = (y1 - y0 + 1) * T;
          const fill = new ImageData(fw, fh), edge = new ImageData(fw, fh);
          const put = (img, i, c) => { img.data[i] = c[0]; img.data[i + 1] = c[1]; img.data[i + 2] = c[2]; img.data[i + 3] = 255; };
          for (let ty = y0; ty <= y1; ty++) for (let tx = x0; tx <= x1; tx++) {
            if (!charged(tx, ty)) continue;
            const oN = !charged(tx, ty - 1), oS = !charged(tx, ty + 1), oW = !charged(tx - 1, ty), oE = !charged(tx + 1, ty);
            for (let y = 0; y < T; y++) for (let x = 0; x < T; x++) {
              if (solid[(PAD + ty * T + y) * W + PAD + tx * T + x]) continue; // blocks stand clear of the wash
              const i = (((ty - y0) * T + y) * fw + (tx - x0) * T + x) * 4;
              put(fill, i, P.zone);
              let d = 99; if (oN) d = Math.min(d, y); if (oS) d = Math.min(d, T - 1 - y); if (oW) d = Math.min(d, x); if (oE) d = Math.min(d, T - 1 - x);
              if (d < 2) put(edge, i, P.zoneLight);
            }
          }
          const mk = (img) => { const cv = document.createElement("canvas"); cv.width = fw; cv.height = fh; cv.getContext("2d").putImageData(img, 0, 0); return cv; };
    ...
    /* ---------- scene ---------- */
    // nothing belongs outside the board's ink frame: overhanging props, labels, glow and motes all stop here
    function clipBoard(c, m) { c.save(); c.beginPath(); c.rect(-m, -m, game.cols * T + m * 2, game.rows * T + m * 2); c.clip(); }
    ...
    function drawScene(c, now) {
      c.setTransform(1, 0, 0, 1, 0, 0); // clear in device space: under the world transform this misses everything above and left of the board
      ...
      if (zoneFx) { // the charged ground pulses; its border counter-pulses so the extent is always readable
        ...
        c.globalAlpha = 0.06 + 0.11 * p;
        c.drawImage(zoneFx.fill, zoneFx.x, zoneFx.y);
        c.globalAlpha = 0.22 + 0.38 * (1 - p);
        c.drawImage(zoneFx.edge, zoneFx.x, zoneFx.y);
        c.restore();
      }
    ...
    function fitView(w, h, margin) {
      const bw = game.cols * T, bh = game.rows * T + RISE;
      const scale = Math.min((w - margin * 2) / bw, (h - margin * 2) / bh);
      return { scale, ox: (w - bw * scale) / 2, oy: (h - bh * scale) / 2 + RISE * scale };
    }
